//*******DAC.c********
//4bit implementation 2 subroutines are written here
// *****DAC_Init******
...
void DAC_Init(void){
    volatile unsigned long counts=0;
	  SYSCTL_RCGC2_R |= 0x20; //activating portF clock
 	  counts=0;
	  counts=1;
	  counts=4;
	  counts=0;
	  PORTF_DIR |= 0xF; //making PF3-0 outputs
	  PORTF_AFSEL &=(~0xF); //clearing alternate functions
	  PORTF_DEN |=0xF; //enables digital I/O for PF3-0
...
// Inputs: 4 bit value to write, stored as "sample"
void DAC_Out(unsigned char sample){
     unsigned long x=0;
      x=PORTF_DATA;
      x &=(~0xF); //masking PF3-0
      x |=sample; //outputing our sample value into PF3-0
      PORTF_DATA=x; //BOOM SHACKALAKA
}	
